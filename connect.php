...
function eReturnRewriteTitleTypeUrl($ttid,$ecms=0){
	...
	$tclassid = "";
	if ($navclassid > 0) $tclassid = $navclassid ."/";
	...
	else
	{
		if($ecms==1)
		{
			$r['pageurl']=$public_r['newsurl'].str_replace(array('[!--ttid--]','[!--classid--]','[!--page--]'),array($ttid,$tclassid,0),$public_r['rewriteinfotype']);
		}
		else
		{
			//NMB
			$r['pageurl']=$public_r['newsurl'].str_replace(array('[!--ttid--]','[!--classid--]'),array($ttid,$tclassid),$public_r['rewriteinfotype']);
		}
		$r['rewrite']=1;
	}
	return $r;
...
function eReturnRewritePageLink($r,$page){
	...
	//NMB
		global $ecms_config;
		$more= $ecms_config['sets']['deftempid'];
		$url=$r['pageurl'].'&page='.$page. ($more>1?"&more=".$more:"");
	}
...
function eReturnRewritePageLink2($r,$page){
	...
	global $ecms_config;
	$more= $ecms_config['sets']['deftempid'];
	$url=$r['pageurl'].'&page='.($page-1).($more>1?"&more=".$more:"");
	}
	return $url;
}
...
function InfoUsePage($num,$line,$page_line,$start,$page,$search,$add){
	...
	if($page<>0)
	{
		...
		$pagepr=$page-1;
		$prepage='<a href="'.eReturnRewritePageLink($add,$pagepr).'" class="prev">'.$fun_r['pripage'].'</a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='&nbsp;<a href="'.eReturnRewritePageLink($add,$pagenex).'" class="next">'.$fun_r['nextpage'].'</a>';
...
'ORIGIN: '. $filepath
		...
		//	curl_setopt($ch, CURLOPT_USERAGENT,"Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; WOW64; Trident/6.0)");
			curl_setopt($ch, CURLOPT_USERAGENT,($_SERVER["HTTP_USER_AGENT"]?$_SERVER["HTTP_USER_AGENT"]:"Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; WOW64; Trident/6.0)"));
if(!strstr($filepath,"https://")) curl_setopt($ch, CURLOPT_REFERER,$filepath);
...
$htmlfp=@fopen($filepath,"r",false, $ctx);
	//远程
	if(strstr($filepath,"://"))
	{
...
function ReturnClassLink($classid){
	...
	for($i=1;$i<count($r)-1;$i++)
	{
		...
		$string.="&nbsp;".$public_r[navfh]."&nbsp;<a href=\"".$url."\"><b>".$class_r[$r[$i]][classname]."</b></a>";
	}
	return $string;
}
...
function ReturnInfoTypeLink($typeid){
    ...
    $string.="&nbsp;".$public_r[navfh]."&nbsp;<a href=\"".$url."\"><b>".$class_tr[$typeid][tname]."</b></a>";
	return $string;
}
...
function sys_ReturnBqInfoTypeUrl($typeid){
	global $public_r,$class_tr,$navclassid;
	//moreport
	if(Moreport_ReturnMustDt())
	{
		$class_tr[$typeid]['listdt']=1;
	}
	if($class_tr[$typeid]['listdt'] || $navclassid > 0)
	{
		$rewriter=eReturnRewriteTitleTypeUrl($typeid,1);
		$url=$rewriter['pageurl'];
	}
...
function ReturnDoOrderF($mid,$orderby,$myorder){
	...
	$orderf=',newstime,id,onclick,totaldown,plnum,truetime';
...
function ReplaceListVars($no,$listtemp,$subnews,$subtitle,$formatdate,$url,$haveclass=0,$r,$field,$docode=0){
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		elseif($f=='titlepic')//标题图片
		{
			if(empty($value))
		    {
				$value=$public_r[newsurl].'e/data/images/notimg.jpg';
			}
			$spf=1;
		}
	...
	$r[smalltext] = str_replace('"','',$r[smalltext]);
	$listtemp=str_replace('[!--smalltextcls--]',strip_tags(nl2br($r[smalltext])),$listtemp);
	$listtemp=str_replace('[!--oldplayer--]',$r[oldplayer],$listtemp);
...
//NMB
	...
	$listtemp=str_replace('[!--bclass.url--]',sys_ReturnBqClassname($class_r[$bclassid],9),$listtemp);
	...
	$listtemp=str_replace('[!--urealurl--]',uReal($titleurl,$r[ftitle]),$listtemp);
	$listtemp=str_replace('[!--ureal--]',uReal("",$r[ftitle]),$listtemp);
...
//NMB UNREAL
function uReal($url,$ftitle) {
	return $url;
	$find = array("第","集","/");
	$replace = array("");
	$ftitle = intval(str_replace($find,$replace,$ftitle));
	$i = strrpos($url,".");
if ($ftitle > 0 && $ftitle < 2010 ) return substr($url,0,$i) ."-".$ftitle. substr($url,$i) ;
else return $url;
