...
//NMB删除模板临时缓存文件
function DelOneTempTmpfile($classid,$gid=1){
global $emoreport_r;
$file=ECMS_PATH.'e/data/tmp/dt_temp'.$classid.'.php';
if ($gid) {
$file=$emoreport_r[$gid][ppath].'e/data/tmp/dt_temp'.$classid.'.php';		
}
	if(file_exists($file))
	{
		DelFiletext($file);
...
function CheckTempGroup($gid){
	...
	//NMB
	return '<B style="color:red">'. $r['gname'] ."</B>";
}
...
function ReplaceKey($newstext,$classid=0){
	...
	//NMB 加入替换先长后短,加入keyurl1 的判断,以先替换TAGS
	$sql=$empire->query("select keyname,keyurl,keyurl1 from {$dbtbpre}enewskey".$where ." order by keyname desc");
	while($r=$empire->fetch($sql))
	{
if ($r["keyurl1"]) $r[keyurl] = $r[keyurl1];
...
//加入TAG表
//NMB 加入 checked  变量,避免未审核信息进入TAG的计数
function eInsertTags($tags,$classid,$id,$newstime,$checked=1){
	...
	//nmb 删除原TAGS
	$empire->query("delete from {$dbtbpre}enewstagsdata where id='$id' and classid = '$classid' and mid='$mid'");
	...
	for($i=0;$i<$count;$i++)
	{
		...
		if($r[tagid])
		{
			...
			if($datar[tagid])
			{
				...
				{
					$empire->query("update {$dbtbpre}enewstagsdata set classid='$classid',newstime='$newstime',checked='$checked' where tagid='$r[tagid]' and id='$id' and mid='$mid' limit 1");
				}
			}
			else
			{
				//NMB 未审核的TAG不计数
				if ($checked) $empire->query("update {$dbtbpre}enewstags set num=num+1 where tagid='$r[tagid]'");
				$empire->query("insert into {$dbtbpre}enewstagsdata(tagid,classid,id,newstime,mid,checked) values('$r[tagid]','$classid','$id','$newstime','$mid','$checked');");
			}
		}
		else
		{
			...
			$tagid=$empire->lastid();
			$empire->query("insert into {$dbtbpre}enewstagsdata(tagid,classid,id,newstime,mid,checked) values('$tagid','$classid','$id','$newstime','$mid','$checked');");
		}
	}
}
...
function CopyImg($text,$copyimg,$copyflash,$classid,$qz,$username,$theid,$cjid,$mark,$fstb=1){
	...
	{
		$r1=explode($exp2,$r[$i]);
		//远程保存加入判断 是否已经是y3600.com的内容
		if(strstr($r1[0],"http://")||strstr($r1[0],"https://"))
	    {
		    if ( strstr($r1[0],"test.y3600.c") || !strstr($r1[0],"y3600.c")) $dourl=$r1[0];
			else $return_r[url] = $r1[0];
		}
		else
	    {
			//是否是本地址
		...
		if ($dourl) $return_r=DoTranUrl($dourl,$classid);
...
function Class_ReplaceSvars($temp,$url,$classid,$title,$key,$des,$classimg,$add,$enews=0){
	...
	//NMB
	$temp=str_replace('[!--self.classurl--]', sys_ReturnBqClassname($class_r[$classid],9),$temp);
	...
	if($enews==0)//栏目
	{
		...
		//NMB
		$temp=str_replace('[!--bclass.url--]',sys_ReturnBqClassname($class_r[$bclassid],9),$temp);
...
function DtNewsBq($classid,$indextext,$ecms=0){
	global $empire,$dbtbpre,$public_r,$emod_r,$class_r,$class_zr,$fun_r,$navclassid,$navinfor,$class_tr,$level_r,$etable_r,$ecms_config;
	$cachetime=$ecms==1?$public_r['dtncachetime']:$public_r['dtcachetime'];
	$file=ECMS_PATH.'e/data/tmp/dt_temp'.$classid.'.php';	
	
//NMB
if ($ecms_config['sets']['selfmoreportid']) {
	global $emoreport_r;
$file=$emoreport_r[$ecms_config['sets']['selfmoreportid']][ppath].'e/data/tmp/dt_temp'.$classid.'.php';		
}
...
function NotinfoListHtml($path,$list_r,$classlevel){
	...
	//NMB img1转换
	$pagetext = user_imgfile($pagetext);
...
function ListHtml($classid,$fields,$enews=0,$userlistr=""){
	...
	//NMB
$field['fr']= array_merge($field['fr'], explode(',','infotags,keyid'));
$field['fcount']=count($field['fr'])-1;
...
$index_r=$empire->fetch1("select id,classid,checked from {$dbtbpre}ecms_".$class_r[$k[classid]][tbname]."_index where id='$k[id]' limit 1");
if(!$index_r['id']||$index_r['classid']!=$k[classid])
{
     
}
else {
//NMB 副表
$infodatatb=ReturnInfoDataTbname($class_r[$k[classid]][tbname],$index_r['checked'],$k['stb']);//返回副表
$query1= "select ".ReturnSqlFtextF($mid)." from ".$infodatatb." where id='$k[id]'";
$finfor=$empire->fetch1($query1);
$finfor["infotags"] = explode(",",$finfor["infotags"]);
$finfor["infotags"] = $finfor["infotags"][0];
//echo strtok($finfor["infotags"], ',');
$k=array_merge($k,$finfor);
...
if ($mpic || $k[mpic]) {
	$fstb = $k[fstb];
	$query1="select fileid,filename,filesize,path,filetime,classid,no,fpath from {$dbtbpre}enewsfile_{$fstb} where id='$k[id]' and classid ='$k[classid]' limit 0,3";
$sql1=$empire->query($query1);
$file["mpic"]="";
	while($r1=$empire->fetch($sql1)) {
	$fspath=ReturnFileSavePath($r1[classid],$r1[fpath]);
	$filepath=$r1[path]?$r1[path].'/':$r1[path];
	$file["mpic"][]=$fspath['fileurl'].$filepath.$r1[filename];
}
$file["mpiccount"] = count($file["mpic"]);
//$file["mpic"] =implode(";",$file["mpic"]);

$k=array_merge($k,$file);
	}	
		//替换列表变量
...
$finfor["infotags"] = $finfor["infotags"][0];
//echo strtok($finfor["infotags"], ',');
$infor=array_merge($infor,$finfor);
...
function GetHtmlRepVar($tempr,$classid){
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		{
			$value='empty('.$value.')?$public_r[newsurl].\'e/data/images/notimg.jpg\':'.$value;
		}
		elseif($f=='title')//标题
		{
		}
	...
	$newstempstr=str_replace('[!--smalltextcls--]','<?=strip_tags(nl2br(str_replace(\'\"\',\'\',$ecms_gr[smalltext])))?>',$newstempstr);
	$newstempstr=str_replace('[!--oldplayer--]','<?=$ecms_gr[oldplayer]?>',$newstempstr);
	//固定变量
	...
	$newstempstr=str_replace('[!--urealurl--]','<?=uReal($grtitleurl,$ecms_gr[ftitle])?>',$newstempstr);
	$newstempstr=str_replace('[!--ureal--]','<?=uReal("",$ecms_gr[ftitle])?>',$newstempstr);
...
function GetHtml($classid,$id,$add,$ecms=0,$doall=0){
	global $public_r,$class_r,$class_zr,$fun_r,$empire,$dbtbpre,$emod_r,$class_tr,$level_r,$etable_r,$tagid_g;
	$mid=$class_r[$classid]['modid'];
	$tbname=$class_r[$classid][tbname];
	if(InfoIsInTable($tbname))//内部表
	{
...
$tagid_g = $add["tagid_g"];
    ...
    //NMB
   $add[newstext] = preg_replace('/small=\\\"1\\\".+?src=\\\"/i','src=\"http://img1.y3600.com/resize.jpg?w=200&h=200&q=',$add[newstext]);
   $add[newstext] = user_imgfile($add[newstext]);	
	
	//$add[newstext] = str_replace('small=\"1\" src=\"','src=\"http://img1.y3600.com/resize?w=200&h=200&q=',$add[newstext]);
//NMB
	if($add[player])
	{
		if(empty($public_r['dorepkey'])&&$docheckrep[4])//替换关键字
		{
			$add[oldplayer] = $add['player'];
			$add[player]=ReplaceKey($add['player'],$add['classid']);
		}	
	}
...
function RepOtherTemp($temptext,$r,$tr){
	...
	if(empty($r[titlepic]))
	{
		$titlepic=$public_r[newsurl]."e/data/images/notimg.jpg";
	}
	else
	{
		$titlepic=$r[titlepic];
...
function ReturnMorepicpathHtml($add){
	...
	/*	if(($p-1)%$line==0||$p==1)
		...
		*/
		...
		$picpath.="<img data-src=\"$showdown_r[1]\" alt=\"$showdown_r[2]\" />";
...
function GetPlTempPage($pltempid=0){
	...
	while($ptr=$empire->fetch($ptsql))
	{
		...
		//NMB img1转换
		$plfiletemp = user_imgfile($plfiletemp);
...
function RepVoteTempAllvar($temptext,$r){
	...
	//NMB
	$temptext=str_replace("[!--vote.class--]",($r[voteclass]?"多选":"单选"),$temptext);
...
function GetPrintPage($printtempid=0){
	...
	while($ptr=$empire->fetch($ptsql))
	{
		...
		//NMB img1转换
		$printtemp = user_imgfile($printtemp);
...
function user_imgfile($string)
{
return preg_replace('/http:\/\/img1\.y3600\.com\/(.+?)(\.jpg)?\?w=(\d+)&h=(\d+)&q=http:\/\/img\.y3600\.com/i','http://img1.y3600.com/z$1/$3/$4',$string);
}
?>