...
function sys_ShowListPage($num,$pagenum,$dolink,$dotype,$page,$lencord,$ok,$search="",$add){
	...
	if($pagenum<>$page)
	{
		...
		$pagenext="<a href='".$nextpagelink."' class='next'>".$fun_r['nextpage']."</a>&nbsp;&nbsp;";
	}
	else
	{
...
function sys_ShowTextPage($totalpage,$page,$dolink,$add,$type,$search=""){
	...
	{
		$pagenex=$page+1;
		$nextpage='&nbsp;<a href="'.$dolink.$add[filename].'_'.$pagenex.$type.'" class="next">'.$fun_r['nextpage'].'</a>';
...
function sys_ShowTextPageSelect($thispagenum,$dolink,$add,$filetype,$n_r){
	...
	for($j=1;$j<=$thispagenum;$j++)
	{
		...
		{
			//NMB
			$title=($add[ftitle]?$add[ftitle]:$add[title]);
...
function sys_ReturnEcmsLoopStext($r){
	...
	$sr['ttname']=$class_tr[$r[ttid]]['tname'];
	return $sr;
}
...
function sys_ShowSearchKey($line,$num,$classid=0,$enews=0){
	...
	$sql=$empire->query("select searchid,keyboard,onclick from {$dbtbpre}enewssearch where iskey=0".$add." order by ".$order." desc limit ".$num);
	$i=0;
	$returnkey="";
	while($r=$empire->fetch($sql))
	{
		...
		$keyurl=$public_r[newsurl]."s/$r[searchid]/";
		$br="";
		if($i%$line==0 && $i < $line)
		{
			$br="<br>";
		...
		$returnkey.="<a href='".$keyurl."' hot='".$r[onclick]."'>".$r[keyboard]."</a>".$jg.$br;
	}
	echo $returnkey;
}
...
function sys_ShowClassByTemp($classid,$tempid,$show=0,$cline=0){
	global $navclassid,$empire,$class_r,$public_r,$dbtbpre,$class_tr;
	//当前栏目
...
//NMB CLASSID 负数时,为标题分类,调用该标题分类相关的母CLASS
	if ($classid < 0) {
		 $ttid = abs($classid);
	$mid=$class_tr[$ttid]['mid'];
	if ($mid) {
		$index_r=$empire->fetch1("select classid from {$dbtbpre}enewsclass where modid='$mid' and bclassid=0 limit 0,1");
		$classid = $index_r['classid'];
}
...
function sys_GetShowClassMore($bclassid,$bcr,$tr,$bno,$bnum,$show=0){
	...
	$bclassimg=$bcr[classimg]?$bcr[classimg]:$public_r[newsurl]."e/data/images/notimg.jpg";//栏目图片
...
function ReplaceShowClassVars($no,$listtemp,$r,$num,$ecms=0,$subnews=0,$docode=0){
	...
	if(empty($r[classimg]))
	{
		$r[classimg]=$public_r[newsurl]."e/data/images/notimg.jpg";
	}
...
function sys_ShowZtData($tempid,$zcid=0,$cline=0,$classid=0){
	...
	if($zcid)
	{
	 ...
	 //NMB 加入SELFINFO
 	global $class_r,$emod_r,$navclassid,$navinfor;    
	if ($zcid == 'selfinfo') {
     $id = '';
     $newsid=intval($navinfor['id']);
     $dh='';
    
     $sql=$empire->query("select ztid from {$dbtbpre}enewsztinfo where id='$newsid' and classid='$navclassid'");
...
//NMB
	...
	while($r=$empire->fetch($sql))
	{
		//替换列表变量
		//NMB
		$repvar=ReplaceShowClassVars($no,$listvar,$r,$num,1,$subnews,$docode);
		//NMB
		$listtext=str_replace("<!--list.var".$changerow."-->",$repvar,$listtext);
		$changerow+=1;
		//超过行数
...
function ReplaceShowPlVars($no,$listtemp,$r,$formatdate,$subnews=0){
	...
	$titlepic=$infor['titlepic']?$infor['titlepic']:$public_r['newsurl'].'e/data/images/notimg.jpg';
...
function sys_ReturnEcmsIndexLoopBq($id=0,$line=10,$enews=3,$classid='',$mid='',$ewhere=''){
	...
	if ($type == 'zt'){
	global $empire,$dbtbpre,$public_r,$class_r,$emod_r,$navclassid,$navinfor;	
if ($id == 'selfinfo' && count($navinfor)>0) {
	$id = '';
	 $newsid=intval($navinfor['id']);
	$dh='';
...
function sys_ReturnTogQuery($type,$id,$line,$classid='',$mid='',$ewhere='',$eorder='',$selectf=''){
	...
	//NMB
	$query='select distinct classid,id'.$selectf.' from '.$table.' where '.$where.' order by '.$orderf.' limit '.$line;
if ($_GET["nmb"])echo $query ;
	$sql=$empire->query1($query);
	if(!$sql)
	{
		echo'SQL Error: '.ReRepSqlTbpre($query);
...
if ($tagid == 'selfinfo') {
	global $navclassid,$navinfor,$tagid_g;

	if ($tagid_g) $tagid = $tagid_g;
	else {
	...
	$sql=$empire->query("select tagid from {$dbtbpre}enewstagsdata where id='$newsid' and classid='$navclassid' order by tid");
...
//-------------------------- 碎片 --------------------------
//显示碎片NMB
function sys_eShowSpInfo($spvar,$line=10,$strlen=0,$classid=""){
	global $empire,$dbtbpre,$public_r,$class_r,$navclassid;;
	if(empty($spvar))
	{
		return '';
	}
	//NMB
	if ($classid == 'selfinfo') $classid = $navclassid;
	else if ($classid == "bclassid") $classid = $class_r[$navclassid]["bclassid"];
	...
	{
		sys_eShowSp1($spr['spid'],$spr,$line,$strlen,$classid);
	}
	elseif($spr['sptype']==2)
	{
...
//静态信息碎片NMB
function sys_eShowSp1($spid,$spr,$line,$strlen,$classid=""){
	global $empire,$dbtbpre,$public_r;
	//取得模板
	...
	//NMB
	$sql=$empire->query("select sid,title,titlepic,bigpic,titleurl,smalltext,titlefont,newstime,titlepre,titlenext from {$dbtbpre}enewssp_1 where spid='$spid'" .$where. " order by newstime desc limit ".$line);
	while($r=$empire->fetch($sql))
	{
		$r[oldtitle]=$r[title];
		//替换列表变量
